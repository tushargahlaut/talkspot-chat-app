  ...
  const accessChat =  async () =>{
    ...
    try {
      ...
      console.log(error)
        ...
        return (
          <div onClick={accessChat} className="flex border cursor-pointer my-3 border-gray-600 rounded-md items-center p-3">
            <div className="w-1/5">
    ...
    const handleSubmitClick = async (e) =>{
        ...
        try{
            ...
            const res1 = await axios.get(
              `http://localhost:8080/api/user?search=${inputRef?.current?.value}`,
              config
            );
