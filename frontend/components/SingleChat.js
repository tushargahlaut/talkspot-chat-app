...
const Header = ({ selectedChat, setIsUpdateGroupChatModal, user, isTyping }) => {
  return (
...
const Footer = ({user,selectedChat,messages,setMessages,socketConnected, typing,setTyping}) =>{
  ...
  return (
    <div className=" w-full bg-emerald-100 h-full items-center flex p-1 my-1">
      ...
      />
      <div onClick={(e)=>handleMessageSend(e)} className="flex items-center cursor-pointer border border-emerald-100">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
  ...
  return (
    <div className="bg-white overflow-hidden flex flex-col border relative p-2 rounded-md border-gray-700 h-full w-full">
      <Header
        ...
        isTyping={isTyping}
      />
      <ChatBox isTyping={isTyping} setIsTyping={setIsTyping} messages={messages}/>
      <div className="w-full">
        <Footer typing={typing} setTyping={setTyping} selectedChat={selectedChat} socketConnected={socketConnected} messages={messages} setMessages={setMessages} user={user} />
      </div>
    </div>
  );
};
