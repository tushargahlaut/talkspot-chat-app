  ...
  return (
    <div className="bg-white border flex flex-col p-2 rounded-md border-gray-700 h-full w-full">
      <div
        ...
        className="border cursor-pointer mb-3 p-3 border-gray-400 bg-gray-200 rounded-md"
      >
        <p>+ New Group Chat</p>
      </div>
      {userList?.map((e) => {
...
const Chats = () => {
  ...
  if (!isLogin) {
    ...
    return <div>Please Go to Home Page and Login</div>;
  }
    ...
    <div className={`${styles.chats}`}>
      <div className="flex h-full overflow-y-auto w-full p-3">
        ...
        <div className="flex-1 p-3">
          {selectedChat && (
            <SingleChat setIsUpdateGroupChatModal={setIsUpdateGroupChatModal} />
          )}
        </div>
      </div>
      {isGroupChatModal && (
